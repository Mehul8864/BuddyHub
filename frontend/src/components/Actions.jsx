...
const Actions = ({ post }) => {
    ...
    const handleShare = (e) => {
        e.preventDefault();
        const url = `${window.location.origin}/${post.postedBy}/post/${post._id}`;
        ...
        <Flex flexDirection="column">
            <Flex gap={3} my={2} onClick={(e) => e.preventDefault()}>
                ...
                <Tooltip label="Copy link">
                    <Box as="span" cursor="pointer" onClick={handleShare}>
                        <ShareSVG />
                    </Box>
                </Tooltip>

                {/* Bookmark */}
                <Tooltip label={bookmarked ? "Remove bookmark" : "Save post"}>
                    <Box as="span" cursor="pointer" onClick={handleBookmark} ml={1}>
                        {bookmarked
                            ? <BsBookmarkFill size={18} color="rgb(237, 73, 86)" />
                            : <BsBookmark size={18} />
                        }
                    </Box>
                </Tooltip>
